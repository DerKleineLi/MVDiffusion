...
class DepthGenerator(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for prompt in batch["prompt"]:
            prompt_embd.append(self.encode_text(prompt, latents_image.device)[0])

        t = torch.randint(
            0,
            self.scheduler.num_train_timesteps,
            (latents_image.shape[0],),
        ...
        denoise = self.mv_base_model(noise_z, t, prompt_embd, batch)
    ...
    def gen_cls_free_guide_pair(
        self, latents, timestep, prompt_embd, batch, type="generation"
    ):
        if type == "interpolation":
            latents_depth = torch.cat(
                [latents, batch["depth_inv_norm_small"][:, :, None]], dim=2
            )
            ...
            meta = {"condition": torch.cat([condition] * 2)}
    ...
    def forward_cls_free(self, latents, _timestep, prompt_embd, batch, model, type):
        ...
        noise_pred = model(_latents, _timestep, _prompt_embd, meta)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.global_rank == 0:
            self.save_image(
                ...
                images,
                batch["prompt"][0],
                batch["depth_inv_norm"].cpu().numpy(),
                batch_idx,
            )
    ...
    def inference_inp(self, batch):
        ...
        prompt_embd = []
        for prompt in batch["prompt"]:
            prompt_embd.append(self.encode_text(prompt[0], latents.device)[0])
        prompt_embd = torch.stack(prompt_embd, dim=1)

        prompt_null = self.encode_text("", device)[0]

        prompt_embd = torch.cat([prompt_null[:, None].repeat(1, m, 1, 1), prompt_embd])
    ...
    def get_inp_image(self, batch, images_pred):
        ...
        batches = []
        images_pred_tensor = (
            torch.tensor(images_pred, device=batch["images"].device) / 127.5 - 1
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        images = ((batch["images"] + 1) * 127.5).cpu().numpy().astype(np.uint8)
        ...
        uvs = batch["uvs"].cpu().numpy().astype(np.float32)
        for i, path in enumerate(image_paths):
            image_id = i

            image_pred_path = os.path.join(output_dir, "{}_pred.png".format(image_id))
            Image.fromarray(images_pred[0, i]).save(image_pred_path)

            image_gt_path = os.path.join(output_dir, "{}_gt.png".format(image_id))
            Image.fromarray(images[0, i]).save(image_gt_path)

            depth_gt_path = os.path.join(output_dir, "{}_depth.png".format(image_id))
            cv2.imwrite(depth_gt_path, depths[0, i])

            poses_path = os.path.join(output_dir, "{}_poses.txt".format(image_id))
            np.savetxt(poses_path, poses[0, i])

            plt.imsave(
                os.path.join(output_dir, "{}_depth_inv.png".format(image_id)),
                depth_inv_norm_full[0, i],
            )
            np.save(
                os.path.join(output_dir, "{}_uv.npy".format(image_id)),
                uvs[0, i],
            )
    ...
    def save_image(self, images_pred, images, prompt, depth_inv_full, batch_idx):
        ...
        with open(
            os.path.join(img_dir, f"{self.global_step}_{batch_idx}.txt"), "w"
        ) as f:
            for p in prompt:
                f.write(p)
        ...
        for m_i in range(images.shape[1]):
            ...
            im.save(
                os.path.join(img_dir, f"{self.global_step}_{batch_idx}_{m_i}_gt.png")
            )
            plt.imsave(
                os.path.join(
                    img_dir, f"{self.global_step}_{batch_idx}_{m_i}_depth_inv.png"
                ),
                depth_inv_full[0, m_i],
            )
